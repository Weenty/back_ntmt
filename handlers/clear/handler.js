    ...
    try {
        ...
        if(role.rows[0].roleId == 1 || role.rows[0].roleId == 3) {
            ...
            for(let i = 0; i < subjects.rows.length; i++) {
                await client.query('DELETE FROM recordbooks WHERE "subjectId" = $1', [subjects.rows[i].id])
                await client.query('DELETE FROM groupssubjects WHERE "subjectId" = $1', [subjects.rows[i].id])
            }
        ...
        const folders = await client.query('SELECT * FROM folders WHERE "userId" = $1', [id])
        for(let i = 0; i < folders.rows.length; i++) {
            await files.deleteFolderRecursive(await files.createStruct(client, folders.rows[i].id), id, client)
        }
